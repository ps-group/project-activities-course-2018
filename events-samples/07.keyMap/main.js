...
function Ball({
    ...
    this.applyForce = function(force, dt) {
        ...
        if (speedVecLength > Ball.MAX_SPEED) {
            this.speed = this.speed.divideScalar(this.speed.length() / Ball.MAX_SPEED);
        }
    }
}
Ball.RADIUS = 0.1;
...
function applyFrictionalForce({ball, dt}) {
    const ANTISPEED_VALUE = 1;
    const antiForce = ball.speed.normalize().multiplyScalar(-1 * ANTISPEED_VALUE);
    if (antiForce.multiplyScalar(dt).length() >= ball.speed.length()) {
        ball.speed = new Vec2(0, 0); 
    }
    else {
        ball.applyForce(antiForce, dt);
    }
}
...
function processKeyMapForBall({ball, keyMap, dt}) {
    ...
    if (wasProcessed) {
        ball.applyForce(directionForce.normalize().multiplyScalar(MOVE_SPEED), dt);
    }

    return wasProcessed;
}

function processKeyMap({ball, keyMap, dt}) {
    processKeyMapForBall({ball, keyMap, dt});
}
